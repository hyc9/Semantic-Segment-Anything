...
def semantic_infer_one_image(img: Image.Image, key, processors, models, rank, preprocess = False):
    ...
    if preprocess:
        ...
        #######补全这一部分  保存valid_mask, class_name
            return 
    else:
...
def main(rank, args):
    ...
    #在preprocess阶段 为每个img的valid_mask.numpy()和class_name 进行一个储存
    
    #补全这部分 读取已经preprocess完成的部分
    ...
    for tar_path in local_files:
        ...
        for idx, item in enumerate(tqdm.tqdm(dataset) if rank == 0 else dataset):
            ...
            key = item["__key__"]
            #如果key已经被处理过，跳过 
            _ = semantic_infer_one_image(item["jpg"], key, processors, models, rank, preprocess = True)
    
            if idx % 100 == 0:
                #保存当前已经处理的结果  文件名以rank区分
    
    #在这一阶段  读取每个rank保存的json文件 将class_name进行一个汇总 生成label2index
    gathered_classnames = xxx
    ...
    #在这一阶段  读取每个img的valid_mask.numpy()和class_name, 生成mask
    for tar_path in (local_files if rank != 0 else tqdm.tqdm(local_files)):
        ...
        with wds.ShardWriter(output_tar_path) as sink:
            for idx, item in enumerate(dataset):
                ...
                mask_arr = semantic_infer_one_image(img, key, processors, models, rank)  
